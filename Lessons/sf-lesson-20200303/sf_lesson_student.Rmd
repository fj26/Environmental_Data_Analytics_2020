...
my_tabular_data_raw <-read.csv(here("data","tabular_data",
                                      "conterm_climate.txt"))
...
ncol(basins_nf_seplains_raw)
length(basins_nf_seplains_raw)
...
# bonus
# add your code here
va_area <- st_area(va_state_bounds_geom) # find the area of NC
va_area
va_centroid <- st_centroid(va_state_bounds_geom) # find the centroid of the NC
va_centroid
va_gages_buffer_100m <- st_buffer(va_gages, dist = 100) # make a buffer around the stream gages in NC
not_va_gages <- st_difference(gages, va_state_bounds_geom) # find stream gages not in NC
```
...
pdf(here("outputs", "va_state_bounds_by_area.pdf"), width = 11, height = 8.5)
ggplot(data = va_state_bounds_geom) +
...
ggplot() +
  ...
  geom_sf(data=nc_gages, color = 'black')+
  theme_bw()
# add your code here
dev.off()
```
...
######!!!!!NEED TO CHANGE STAID FROM NUM TO CHARACTER TO KEEP TWO DATASET SAME DATA TYPE
