...
O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray() ##background color - gray
print(O3plot1)
...
O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light() ##grey outline of the plot
print(O3plot3)

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() ##remove everything within the plot, just axes, not change legend
print(O3plot4) 
```
...
theme_set(mytheme) ## set at the beginning of the session could have all plots follow this theme
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + ##lty 2 is dash line
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + ##put point behind the line (the next layer, alpha -- transparency)
  ...
  ylab(expression("O"[3]* " AQI Value")) + ##3 is the subscript
...
NvsP2 <-
  ...
  labs(y = expression(paste("TN (", mu, "g/L)")),
       x= expression(paste("TP (",mu, "g/L)")),
       color = "Depth (m)", shape = "") + # change your legend labels here
  ...
  #ylab(expression(paste("TN (", mu, "g/L)"))) +
  #xlab(expression(paste("TP (",mu, "g/L)")))
print(NvsP2)
...
# place your additional edits here
  labs(y = expression(paste("Concentration (", mu, "g/L)")), x = "" ,
color = "Nutrients", shape = "") + 
