...
#setwd("/Users/Victoria/Environmental_Data_Analytics_2020")
```
...
```{r, message = FALSE}
#message = FALSE will not print out output in the pdf, but the script in this window still runs 
#Load package
library(tidyverse)
```
...
##./ -> go to one file in
##right click file in the folder, hold on option, the file name will have the option copying filename
##2 dot means going to previous level
...
> ANSWER: This metadata provides a detailed explanation of the meaning of each column when the column name is brief. The source of the dataset is also provided. The format of data file name is also explained in the metadata. Also description of the meaning of the column names.

```{r}
View(USGS.flow.data)
# Alternate option: click on data frame in Environment tab

class(USGS.flow.data) ##different types of data included; two dimensional
colnames(USGS.flow.data)

# Rename columns - have to list all column names in order to rename column names
##if only rename a few column, put the number of the column, eg: colnames(xx)[3], change the 3rd columnm name
...
str(USGS.flow.data) #structure of the data frame
dim(USGS.flow.data)
length(USGS.flow.data) # number of columns, no function of width, only get width from dim or nrow
...
class(USGS.flow.data$datetime) #factor class is a type of data -- specifically categorical data;  
...
> Answer: Numerical value becomes "NA", other factors become "Factor" with "A" and "P" and "1"
...
> Answer: duplication analysis to remove NAs
...
#write a function using if else to change year value from 2027 to 1927
create.early.dates <- (function(d) {
       paste0(ifelse(d > 181231,"19","20"),d)
       })
?paste0

#run the function created above using datatime in the csv file
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

#reform year month day in the data frame
