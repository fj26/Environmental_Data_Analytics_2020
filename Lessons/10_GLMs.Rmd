...
EPAair.subsample <- sample_n(EPAair, 5000) ##take 5000 variables in a new sample

# Evaluate assumption of normal distribution
shapiro.test((EPAair.subsample$Ozone)) ##non-hypthesis: nomally distributed -- p value is small -- data is not a well normall distribution
...
O3.onesample
#one side t-test -- compare to a single value "mu"
##alternative hypothesis: mean is less than 50
##t < -1.96 or t > 1.96 ----> p < 0.05 --> reject non hypothesis
##within 95% intervel, the sample mean is 40.87526
...
var.test(EPAair$Ozone ~ EPAair$Year) ## variance > 1 -- signficantly different from each other

ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_freqpoly() ## as.factor(Year) makes year as a continuous variable not a discrete variable

# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year)
O3.twosample ##test ozone by year -- alternative hypothesis: if the means are different -- p< 0.05 and t<-1,96 -- reject non-hypothesis
...
plot(O3.twosample2)
par(mfrow = c(2,2)) ##display 4 plots in one picture
```
...
Ozone.density <- ggplot(EPAair, aes(x = Ozone, fill = as.factor(Year))) +
  ...
  #ggplot(EPAair) +
  #geom_density(aes(x= Ozone, fill = as.factor(EPAair$Year)),alpha = 0.5)
print(Ozone.density)

Ozone.box <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone)) +
  geom_boxplot() +
  ...
  #ggplot(EPAair)+
  #geom_boxplot(aes(x=as.factor(EPAair$Year), y= EPAair$Ozone))
print(Ozone.box)

Ozone.voilin <- ggplot(EPAair, aes(x = as.factor(Year), y = Ozone))+
  geom_violin()+
  geom_hline(yintercept = quantile(na.omit(EPAair$Ozone),0.5), color = "#238b45", lty = 2, size = 0.9)+
...
## 0.85 from previous test (difference between 2019 and 2018)
## W from wilcoxon test
