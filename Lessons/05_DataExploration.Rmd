...
# Check our date column - R cannot tell csv file data type
class(USGS.flow.data$datetime)
##tell R datetime is in the date format and is a date type data
...
```{r, fig.height = 3, fig.width = 4}
#the number of discarged mean value 
...
#have a fixed number of bins in histogram,and value divided into these numbers
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

#set x-axis scale from 0 to 500 with 10-unit bin width
...
```{r, fig.height = 3, fig.width = 4}
#Have several columns displayed together - using frequncy polygon
ggplot(USGS.flow.data) +
...
summary(USGS.flow.data$gage.height.mean) ##get the number of NAs
summary(USGS.flow.data$gage.height.mean.approval)

#add legend to the frequency polygon
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#cannot show the difference for different sample size
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#can specify the distribution of y
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#violin plot shows sample counts
ggplot(USGS.flow.data) +
  geom_violin(aes(x = gage.height.mean.approval, y = gage.height.mean), 
              draw_quantiles = c(0.25, 0.5, 0.75),
              scale = 'area') ##same area in total, just in proportional value of percentage - see help for notes about "count" and "width" options
```
