...
# to avoid other lake names, use droplevels to remove those 0 data - did not change dimension, but only remain Paul and Peter lake
...
NTL.nutrient.data.PeterPaul$sampledate <- as.Date(NTL.nutrient.data.PeterPaul$sampledate, format = "%m/%d/%y") #the format it came in

NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #
  mutate(month = month(sampledate)) %>% #insert the number of month
  select(lakeid:daynum, month, sampledate:comments) %>% # 
  drop_na(depth) ## drop any NA value in the depth column
...
NTL.nutrient.data.PeterPaul.subset <- select(NTL.nutrient.data.PeterPaul.subset,
...
## subset is a function removing rows -- will change dimension
## select is removing/selecting columns
...
NTL.PeterPaul.summaries <- 
  ...
  filter(depth == 0) %>% ## filter surface data so depth = 0
