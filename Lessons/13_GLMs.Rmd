...
## Intercept -- Paul lake nutrient level at surface level (depth =0) --> Peter lake  = intercept+Peterlake; Depth means each meter increase in depth, nutrients in Paul lake will increase by xxx
## no difference in formula from Anova

# interaction effects
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction)

##Peter:depth -- for each meter increase in Peter lake, the nutrient level will increase xxx (if negative, means decrease)

TNplot <- ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = tn_ug, color = lakename)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  xlim(0, 10)+
  labs(x='Depth (m)', y= expression(paste("TN (", mu, "g/L)")), color = '')+
...
ggplot(NTL.summertemp, aes(x = year4, y = temperature_C)) +
  ...
  scale_color_viridis_d() + 
  geom_smooth(method = 'lm', se = FALSE)+##BEST fit line for the fixed model -- this is linear model
