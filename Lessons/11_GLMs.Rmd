...
## if p-value is < 0.5 -- not fit to a normal distribution
...
##p-value is low -- these values are not close to each other
...
# Format ANOVA as lm - lm is interceptual
## the actual value for iea ID is intercept value +/- estimate std.
...
# Post-hoc test - only works for aov, not for lm
## looks for p value less than 0.5
TukeyHSD(Litter.Totals.anova)

# Extract groupings for pairwise relationships
## if share the same letter -- not significantly different from other
...
Litter.Totals.plot <- ggplot(Litter.Totals, aes(x = plotID, y = dryMass)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + ##change a new theme
  stat_summary(geom = "text", fun.y = max, vjust = -1, size = 3.5,
               label = c("abc", "ab", "abcd", "bcd", "bcd", "a", 
                         "d", "abcd", "cd", "d", "abcd", "abcd")) + ## geom =  what type of geommetry you want to use. fun.y = where you want to put this text
...
> Total litter biomass differed among sites at the Niwot Ridge LTER (Figure 1; ANOVA, F11,198 = 4.813, p< 0.0001).
## the greater F value from 0, the more signifcantly different from the group
