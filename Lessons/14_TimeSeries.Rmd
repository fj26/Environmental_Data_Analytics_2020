...
EnoDischarge.complete <- EnoDischarge %>%
  drop_na(discharge.mean) ##only drop na from discharge mean -- single column of the data; na.omit and na.exclude do the same thing
...
##frequency -- equal interval for each year
```
...
###trend is the annual average
###data = seansonal + trend + remainder (scales for seasonal and trend are too small to affect data trend -- similar to remainder trend)
###seasonal across the year -- difference among years-- which is not differentiated among years as they are similar and too noised -- overall seasonal effect explained year to year
...
##stl does not adjust actual bonds of the value, but just actual value
...
##p-value is low -- reject Non-hypothesis that this is normal distribution
...
>  Mean daily discahrge in the Eno River from 1927 to 1971 was significatly higher than mean daily discharge from 1985 to 2019 (Wilcoxon test, W = 1.17e+08, p-value < 2.2e-16).
Can use boxplot to show different mean location.
...
> aggregation: from monthly to year data (from multiple points to single points)

> subsampling: subsetting -- first set of the entire dataset; take representative group (random samples) from the entire group -- use the least

> interpolation: predicting what is in the data gap based on existed data
...
EnoDischarge.late.monthly <- EnoDischarge.late %>%
  ...
  group_by(Year, Month) %>%
  summarise(Discharge = mean(discharge.mean))  ## mean takes care of outliers
  
EnoDischarge.late.monthly$Date <- as.Date(paste(EnoDischarge.late.monthly$Year,
                                                EnoDischarge.late.monthly$Month, 
                                                1, sep="-"), 
                                          format = "%Y-%m-%d") ## write a function to insert Date
...
# Inspect results
EnoDischarge.late.trend ##reveals if there is a signficant seasonal trend - non-hypothesis: there is no signficicante trend -- p-value is high, so there is no significant trend, but not signficantly different from the trend overall (since the z value is small)
...
> There is no significant trend, and no signficantly difference from the overall trend (Seasonal Mann-Kendall trend test (Hirsch-Slack test)
z=-0.13967, p-value = 08889)
