...
> Add notes here: Mitigation, adaptation, ecohealth
...
# matrix filtering - rows = 0 and every single column
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]

# dplyr filtering - which row to grab
...
summary(NTL.phys.data$lakename) ##how many sample cells in each lake
...
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) ##specify any new samples are within paul and peter

# Choose a range of conditions of a numeric or integer variable 
summary(NTL.phys.data$daynum) ## 55 as min - the 55th day of that year is the first day to collect data
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) ## daynum - the day number from Jan 1 of the year
...
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304)) ## c - means including

# Exercise: 
# filter NTL.phys.data for the year 1999
NTL.phys.data.Year99 <- filter(NTL.phys.data, year4 == 1999)
...
NTL.phys.data.Year9099 <- filter(NTL.phys.data, lakename == 'Tuesday Lake', year4 %in% c(1990:1999))

dim(NTL.phys.data.Year9099)
dim(NTL.phys.data.Year99)
```
Question: Why don't we filter using row numbers?

> Answer: filter based on attributes instead of counting row number
...
```{r}
##name which columns gonna be left in the dataset 
##select only applies to name of the column
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C)
## do not include something - use "-", eg: -lakename -- exclude lakename column
```
### Mutate
...
## month is the default/ build-in element in lubridate
