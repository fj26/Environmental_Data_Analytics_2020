...
```{r}
tempplot <- ggplot(PPchem, aes(x = month, y = temperature_C, color = lakename))+
  geom_boxplot()+
  labs(y = 'Temperature (C)',
       x= 'Month')

tnplot <- ggplot(PPchem, aes(x = month, y = tn_ug, color = lakename))+
  geom_boxplot()+
  labs(y = expression(paste("TN (", mu, "g/L)")),
       x= 'Month')

tpplot <-  ggplot(PPchem, aes(x = month, y = tp_ug, color = lakename))+
  geom_boxplot()+
...
> Answer: Data are available in summer (from May to October), and Paul Lake has fewer outliers than Peter lake for nutrients, while there is no outlier for temperature data. 
