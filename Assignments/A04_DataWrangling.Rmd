...
#5
PM25.2018 <- mutate(PM25.2018, AQS_PARAMETER_DESC = 'PM2.5' )
PM25.2019 <- mutate(PM25.2019, AQS_PARAMETER_DESC = 'PM2.5' )
...
```{r}
#7
EPAair <- rbind(O3.2018, O3.2019, PM25.2018, PM25.2019)
...
dim(EPAair.select)
#9
EPAair.select.unique <- spread(EPAair.select, AQS_PARAMETER_DESC, meanAQI)
#10
dim(EPAair.select.unique)
#11
write.csv(EPAair.select.unique, row.names = FALSE, file = '../Data/Processed/EPAair_O3_PM25_NC1718_Processed.csv')
```
...
```{r}
#12a
EPAair.select.unique.AQI <-
  EPAair.select.unique %>%
...
> Answer: If na.omit removes cases, the row numbers of the cases form the "na.action" attribute of the result, of class "omit". Also, 'na.omit' cannot select specific variables - it will omit all N/A values. drop.na will drop rows containing missing values based on our selection.
