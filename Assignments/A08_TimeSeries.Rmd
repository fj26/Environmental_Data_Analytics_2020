...
# 4
GaringerOzonesel <-GaringerOzone %>%
select(Date, Daily.Max.8.hour.Ozone.Concentration, DAILY_AQI_VALUE)

# 5
Days <- as.data.frame(seq(as.Date("2010-01-01"), as.Date("2019-12-31"), "day"))
names(Days) <- paste("Date")
names(Days)

# 6
GaringerOzone <- left_join(Days, GaringerOzonesel)
...
> Answer: Ozone concentration data is at daily scale, which might be but should not be same everyday, so piecewise interpolation, which assumes missing to be equal to the measurement made nearest to that date is not accurate enough. A spline interpolation, a quadratic function is used to interpolate rather than directly filling the missing data with a straight line, is an alternative to a linear interpolation, but it is more appropriate for higher-degree polynomials used in polynomial interpolation.
...
> Answer: The assumptions and characteristics of the dataset for Seasonal Mann-Kendall are: seasonality, non-parametric, no temporal autocorelation, identical distribution. From the ggplot in Q7, there is a seasonal cycle for the ozone concentration each year, and our study question is to know if the concentration has changed over the time with the consideratin of the seasonal component.Other tests for the monotonic trend analysis do not address on seasonality. Therefore, Seasonal Mann-Kendall is the most appropriate test.
...
GaringerOzone.interpolated <- ggplot(GaringerOzone, aes(x = Date, y= Daily.Max.8.hour.Ozone.Concentration))+
  geom_point()+
...
GaringerOzone.monthly$Date <- as.Date(paste(GaringerOzone.monthly$Year,
                             ...
                             1, sep="-"), format = "%Y-%m-%d")
# 10
GaringerOzone.monthly.ts <- ts(GaringerOzone.monthly$Concentration, frequency = 12, start = c(2010,1,1), end = c(2019,12,1))

# 11
GaringerOzoneTrend <- smk.test(GaringerOzone.monthly.ts) 
GaringerOzoneTrend
summary(GaringerOzoneTrend)
...
> Answer: 
There is a significant seasonal decreasing (negative) trend of ozone concentrations over the 2010s at this station. Also, there is a signficant difference from the overall trend. The observations of the trend in recent years tend to be smaller than those in earlier years. (Seasonal Mann-Kendall trend test z=-1.963, p-value = 0.04965).